...
teacher_banner = """
        ...
        <div style="background: linear-gradient(135deg, rgba(21,101,192,0.1) 0%, rgba(21,101,192,0.05) 100%); border: 1px solid var(--brand-1); border-radius: var(--r-xl); padding: 24px; margin-bottom: 24px; display: flex; align-items: center; justify-content: space-between;">
          <div>
            ...
            <p style="margin: 0; color: var(--text-2); font-size: 0.95rem;">Créez et surveillez des examens et interrogations avec chronomètre strict et anti-triche simulé.</p>
          </div>
          <button class="btn btn-primary" onclick="window.navigateToView('composition')">Découvrir l'interface Professeur</button>
        </div>
"""
...
# Inject into teacherView
content = content.replace('<div id="teacherView" style="display:none;">', '<div id="teacherView" style="display:none;">\n' + teacher_banner)

# Inject into learnerView
content = content.replace('<div id="learnerView" style="display:none;">', '<div id="learnerView" style="display:none;">\n' + learner_banner)
